...
# === Bagging Classifier with Linear SVM Base ===
n_estimators_list = [10, 20]
best_f1 = 0
best_model = None
bagging_results = []

print("\n👜 Bagging Classifier with Linear SVM:")
for n in n_estimators_list:
    base_svm = SVC(kernel="linear", C=1)
    model = BaggingClassifier(estimator=base_svm, n_estimators=n, random_state=42)

    model.fit(X_train, y_train)
    y_pred = model.predict(X_val)
    f1 = f1_score(y_val, y_pred)

    print(f"n_estimators={n:<2} → F1 Score = {f1:.4f}")
    bagging_results.append({'n_estimators': n, 'F1_Score': f1})

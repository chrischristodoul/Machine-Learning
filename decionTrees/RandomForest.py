...
# Create a directory to store all trees
...
for i, estimator in enumerate(best_model.estimators_):
    dot_data = export_graphviz(
        estimator,
        out_file=None,
        feature_names=[f"pixel_{j}" for j in range(X.shape[1])],
    ...
    filename = f"rf_trees/tree_{i}"
    graph.render(filename, format="png", cleanup=True)  # Saves tree_i.png

print(f"\n Exported {len(best_model.estimators_)} trees to 'rf_trees/' directory.")
